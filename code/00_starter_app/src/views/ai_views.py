...
@router.get('/ai/start/{action}/{podcast_id}/{episode_number}')
...
@router.get('ai/check_status/{job_id}')
...
async def check_job_status(request: Request, job_id: str):
    ...
    if not vm.job.is_finished:
        print(f'The job {vm.job_id} is still running')
        # return fastapi_chameleon.response( template_file: 'ai/job_completed.html', **vm.to_dict())
        return fastapi_chameleon.response( 'ai/job_completed.html', **vm.to_dict())

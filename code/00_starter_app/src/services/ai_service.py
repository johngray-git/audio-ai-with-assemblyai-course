...
async def worker_transcribe_episode(podcast_id: str, episode_number: int) -> EpisodeTranscript:
    ...
    if not episode:
        raise Exception(f"Episode not found for ID {podcast_id} and episode {episode_number}")

    mp3_url = episode.enclosure_url
    print(f"Ww are transcribing {podcast.title} - {episode.title} from {mp3_url} ...")

    transcriber = asssemblyai.Transcriber()
    config = assemblyai.TranscriberConfig(
        punctuate=True,
        format_text=True,
    ...
    if not db_transcript.succesful:
        msg = (
    ...
    print(f'Processing complete for the transcription, dt = {dt.total_seconds(),.2} sec.')
